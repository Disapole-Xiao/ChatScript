...
export interface Config {
   ...
   * @param varname - 变量名
   * @param userId - 用户ID
   * @returns 变量值，输出时会调用 toString() 方法转化为字符串
   */
...
export class Interpreter implements Config {
   ...
   * 创建一个新的解释器实例
   * @param script - 要执行的脚本
   * @param config - 配置对象，包含回调函数
   * @see {@link Config}
   * @param userId - 用户ID，机器人客服会在调用上述方法时带上初始传入的 userId。没有传入则为 undefined
   */
  constructor(script: Script, config: Config, userId?: string) {
    this.script = script;
   ...
   *
   * @param fromProcId - 起始过程ID，默认从 entryProcId 开始
   * @throws Error 当会话已经在运行时
   * @throws {@link RuntimeError} 当指定的过程不存在时
   */
   ...
   * @private
   * @throws {@link RuntimeError} 当变量未定义或过程未定义时
   */
