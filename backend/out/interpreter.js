...
class Interpreter {
    constructor(script, config, variables) {
        this.timers = [];
        this.isWorking = true; // 会话是否正在运行
        this.taskQueue = []; // 任务队列减少递归，每个任务是一个函数
        this.script = script;
        ...
        try {
            if (this.isWorking) {
                ...
                if (!this.script.procs.has(this.script.entryProcId)) {
                    throw new error_1.RuntimeError(0, `Porcedure ${this.script.entryProcId} 未定义`);
                }
    ...
    receive(message) {
        ...
        for (const hearEvent of this.curProc.hearEvents || []) {
            if ((typeof hearEvent.pattern === 'string' && message.includes(hearEvent.pattern)) ||
            ...
            for (const action of actions) {
                switch (action.type) {
                        ...
                        for (const token of action.tokens) {
                            if (token.type === 'string') {
                                ...
                                if (this.variables.has(token.content)) {
                                    ...
                                    throw new error_1.RuntimeError(action.lineIdx, `变量 ${token.content} 不存在`);
                                }
                            }
                        }
                    ...
                    case 'gotoAction':
                        ...
                        if (!nextProc)
                            throw new error_1.RuntimeError(action.lineIdx, `Porcedure ${action.procId} 未定义`);
